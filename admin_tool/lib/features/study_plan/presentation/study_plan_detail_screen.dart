...
final planWeeksProvider = StreamProvider.family<List<StudyPlanWeek>, String>((ref, planId) {
...
class StudyPlanDetailScreen extends ConsumerWidget {
              ...
              return Card(
                ...
                child: ExpansionTile(
                  leading: CircleAvatar(
                    ...
                    child: Text('${week.weekNumber}', style: const TextStyle(color: Colors.white)),
                  ),
                  title: Row(
                    children: [
                      Expanded(child: Text(week.title, style: const TextStyle(fontWeight: FontWeight.bold))),
                      IconButton(
                  ...
                  subtitle: Text('${week.days.length} ngày • ${week.days.fold(0, (sum, day) => sum + day.activityIds.length)} hoạt động'),
                  children: week.days.asMap().entries.map((entry) {
                    ...
                    return ListTile(
                      ...
                      subtitle: _ActivitiesSummary(activityIds: day.activityIds),
                      trailing: const Icon(Icons.add_circle_outline, size: 16),
                      onTap: () => _manageDayActivities(context, ref, week, dayIndex),
                    );
                  }).toList(),
                ),
              );
  ...
  Future<void> _editWeekTitle(BuildContext context, WidgetRef ref, StudyPlanWeek week) async {
  ...
  Future<void> _manageDayActivities(BuildContext context, WidgetRef ref, StudyPlanWeek week, int dayIndex) async {
    await showDialog(
      context: context,
      builder: (context) => StudyDayActivityManagementDialog(
        week: week,
        dayIndex: dayIndex,
      ),
    );
...
class _ActivitiesSummary extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return activitiesAsync.when(
      data: (allActivities) {
        final titles = activityIds.map((id) {
          final activity = allActivities.firstWhere((a) => a.id == id, orElse: () => const Activity(id: '', title: 'Unknown', type: ActivityType.breakTime));
...
class StudyDayActivityManagementDialog extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return weeksAsync.when(
      data: (weeks) {
        ...
        final updatedWeek = weeks.firstWhere((w) => w.id == week.id, orElse: () => week);
        ...
        return AlertDialog(
          ...
          content: SizedBox(
            ...
            child: allActivitiesAsync.when(
              data: (allActivities) {
                final currentActivities = day.activityIds.map((id) => 
                  allActivities.firstWhere((a) => a.id == id, orElse: () => Activity(id: id, title: 'Unknown ID: $id', type: ActivityType.breakTime))
                ).toList();

                return Column(
                  children: [
                    const Text('Danh sách hoạt động trong ngày:', style: TextStyle(fontWeight: FontWeight.bold)),
                    Expanded(
                      flex: 1,
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return ListTile(
                            ...
                            trailing: IconButton(
                              ...
                              onPressed: () => _removeActivity(ref, updatedWeek, index),
                            ),
                          );
                        },
                      ),
                    ...
                    const Divider(),
                    const Text('Thêm hoạt động mới:', style: TextStyle(fontWeight: FontWeight.bold)),
                    Expanded(
                      flex: 1,
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          if (day.activityIds.contains(activity.id)) return const SizedBox();
                          return ListTile(
                            dense: true,
                            leading: _getTypeIcon(activity.type),
                            title: Text(activity.title),
                            trailing: const Icon(Icons.add),
                            onTap: () => _addActivity(ref, updatedWeek, activity.id),
                          );
                        },
                      ),
                    ),
  ...
  void _addActivity(WidgetRef ref, StudyPlanWeek currentWeek, String activityId) {
    final updatedDays = [...currentWeek.days];
    final day = updatedDays[dayIndex];
    updatedDays[dayIndex] = day.copyWith(activityIds: [...day.activityIds, activityId]);
  ...
  Icon _getTypeIcon(ActivityType type) {
    switch (type) {
      case ActivityType.lesson: return const Icon(Icons.book, size: 20);
      case ActivityType.music: return const Icon(Icons.music_note, size: 20);
      case ActivityType.exercise: return const Icon(Icons.edit_note, size: 20);
      case ActivityType.communication: return const Icon(Icons.chat, size: 20);
      case ActivityType.writing: return const Icon(Icons.create, size: 20);
      case ActivityType.observation: return const Icon(Icons.visibility, size: 20);
      case ActivityType.movie: return const Icon(Icons.movie, size: 20);
      case ActivityType.breakTime: return const Icon(Icons.hourglass_empty, size: 20);
    }
  }
}
