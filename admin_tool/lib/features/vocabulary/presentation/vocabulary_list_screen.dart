...
class VocabularyListScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Vocabulary Management', style: TextStyle(fontWeight: FontWeight.bold)),
        backgroundColor: Colors.white,
        elevation: 0,
        actions: [
          IconButton(
  ...
  Widget _buildFilterBar(BuildContext context, WidgetRef ref, VocabularyFilterState filters, List lessons) {
    return Container(
      ...
      child: Row(
        children: [
          Expanded(
            flex: 2,
            child: TextField(
              onChanged: (val) => ref.read(vocabularyFilterProvider.notifier).updateSearch(val),
          ...
          Expanded(
            child: DropdownButtonFormField<String>(
              ...
              items: [
                ...
                const DropdownMenuItem(value: 'unknown', child: Text('Unknown (No Lesson)')),
                ...lessons.map((l) => DropdownMenuItem(value: l.id, child: Text(l.title, overflow: TextOverflow.ellipsis))),
              ],
              onChanged: (val) => ref.read(vocabularyFilterProvider.notifier).updateLesson(val),
            ),
          ),
          const SizedBox(width: 16),
          Expanded(
            child: TextField(
              onChanged: (val) => ref.read(vocabularyFilterProvider.notifier).updateCategory(val),
              decoration: InputDecoration(
  ...
  Widget _buildTable(BuildContext context, WidgetRef ref, List<Vocabulary> items, VocabularyFilterState filters, List lessons) {
    if (items.isEmpty) {
  ...
  Widget _buildPagination(WidgetRef ref, VocabularyFilterState filters, int totalItems, int totalPages) {
    return Padding(
      padding: const EdgeInsets.all(16.0),
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          Text('Total: $totalItems items', style: const TextStyle(color: Colors.grey)),
          Row(
            children: [
              IconButton(onPressed: filters.pageIndex > 0 ? () => ref.read(vocabularyFilterProvider.notifier).setPage(filters.pageIndex - 1) : null, icon: const Icon(Icons.chevron_left)),
              Text('Page ${filters.pageIndex + 1} of $totalPages'),
              IconButton(onPressed: filters.pageIndex < totalPages - 1 ? () => ref.read(vocabularyFilterProvider.notifier).setPage(filters.pageIndex + 1) : null, icon: const Icon(Icons.chevron_right)),
            ],
          ),
        ],
      ),
  ...
  Future<void> _confirmDelete(BuildContext context, WidgetRef ref, Vocabulary voc) async {
