...
class UserRepository {
  ...
  CollectionReference<UserProfile> get _collection =>
      _firestore.collection('users').withConverter<UserProfile>(
            fromFirestore: (snapshot, _) => UserProfile.fromJson(snapshot.data()!..['id'] = snapshot.id),
            toFirestore: (user, _) => user.toJson()..remove('id'),
          );

  Future<void> createUser(UserProfile user) => _collection.doc(user.id).set(user);

  Future<void> updateUser(UserProfile user) => _collection.doc(user.id).set(user);

  Stream<List<UserProfile>> getUsers() =>
