...
class _ActivityFormDialogState extends ConsumerState<ActivityFormDialog> {
  ...
  void initState() {
    ...
    _mediaUrlController = TextEditingController(text: widget.activity?.mediaUrl ?? '');
    _linkUrlController = TextEditingController(text: widget.activity?.linkUrl ?? '');
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              if (_type == ActivityType.lesson)
                lessonsAsync.when(
                  data: (items) => DropdownButtonFormField<String>(
                    ...
                    items: items.map((i) => DropdownMenuItem(value: i.id, child: Text(i.title))).toList(),
              ...
              if (_type == ActivityType.exercise)
                exercisesAsync.when(
                  data: (items) => DropdownButtonFormField<String>(
                    ...
                    decoration: const InputDecoration(labelText: 'Target Exercise', border: OutlineInputBorder()),
                    items: items.map((i) => DropdownMenuItem(value: i.id, child: Text(i.title))).toList(),
              ...
              if (_type == ActivityType.music || _type == ActivityType.movie) ...[
                TextFormField(
                  controller: _mediaUrlController,
                  decoration: const InputDecoration(labelText: 'Media URL / Link', border: OutlineInputBorder()),
                ),
                const SizedBox(height: 16),
              ],
              TextFormField(
