"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.StudyPlanController = void 0;
const common_1 = require("@nestjs/common");
const study_plan_service_1 = require("./study-plan.service");
const update_study_plan_dto_1 = require("./dto/update-study-plan.dto");
const auth_guard_1 = require("../auth/guards/auth.guard");
const admin_guard_1 = require("../auth/guards/admin.guard");
const swagger_1 = require("@nestjs/swagger");
let StudyPlanController = class StudyPlanController {
    studyPlanService;
    constructor(studyPlanService) {
        this.studyPlanService = studyPlanService;
    }
    getStudyPlan() {
        return this.studyPlanService.getStudyPlan();
    }
    updateStudyPlan(dto) {
        return this.studyPlanService.updateStudyPlan('suomen_mestari_1', dto);
    }
};
exports.StudyPlanController = StudyPlanController;
__decorate([
    (0, common_1.Get)('studyPlan'),
    (0, common_1.UseGuards)(auth_guard_1.AuthGuard),
    (0, swagger_1.ApiOperation)({ summary: 'Get study plan' }),
    (0, swagger_1.ApiResponse)({ status: 200, description: 'Study plan details' }),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", void 0)
], StudyPlanController.prototype, "getStudyPlan", null);
__decorate([
    (0, common_1.Put)('admin/studyPlan'),
    (0, common_1.UseGuards)(admin_guard_1.AdminGuard),
    (0, swagger_1.ApiOperation)({ summary: 'Update study plan' }),
    (0, swagger_1.ApiResponse)({ status: 200, description: 'Study plan updated' }),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [update_study_plan_dto_1.UpdateStudyPlanDto]),
    __metadata("design:returntype", void 0)
], StudyPlanController.prototype, "updateStudyPlan", null);
exports.StudyPlanController = StudyPlanController = __decorate([
    (0, swagger_1.ApiTags)('Study Plan'),
    (0, swagger_1.ApiBearerAuth)(),
    (0, common_1.Controller)(),
    __metadata("design:paramtypes", [study_plan_service_1.StudyPlanService])
], StudyPlanController);
//# sourceMappingURL=study-plan.controller.js.map