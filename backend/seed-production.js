...
if (!admin.apps.length) {
  admin.initializeApp({
    ...
    projectId: 'hoc-tieng-phan' // or serviceAccount.project_id
  });
}
...
async function fullSeed() {
  ...
  // Dọn dẹp trong loop nhưng chỉ lấy top 500 nếu cần, ở đây delete tuần tự
  ...
  for (const item of vocabularyData) {
    ...
    vocabCount++;
    if (vocabCount % 200 === 0) { // giảm kích thước batch để tránh Firestore limit
      await batch.commit();
      batch = db.batch();
  ...
  console.log('🎉 TOÀN BỘ CURRICULUM ĐÃ ĐƯỢC SEED PRODUCTION THÀNH CÔNG!');
}

fullSeed().catch(err => {
  console.error('❌ Seeding production failed:', err);
  process.exit(1);
});
